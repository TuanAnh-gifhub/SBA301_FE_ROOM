...
const roomsService = {
  createRoom: async (
    ...
    const response = await api.post<ApiResponse<RoomResponse>>(
      `/rooms/${rentalAreaId}`,
      formData,
    );
  ...
  getMyRooms: async (): Promise<ApiResponse<any[]>> => {
    const response = await api.get<ApiResponse<any[]>>(`/rooms/me`);
    return response.data;
  },

  getRoomsByRentalArea: async (
    rentalAreaId: string,
  ): Promise<ApiResponse<RoomCardResponse[]>> => {
  ...
  getRoomDetail: async (roomId: string): Promise<ApiResponse<RoomResponse>> => {
    const response = await api.get<ApiResponse<RoomResponse>>(
      `/rooms/${roomId}`,
    );
    return response.data;
  },
  ...
  updateRoom: async (
    ...
    const response = await api.put<ApiResponse<RoomResponse>>(
      `/rooms/${roomId}`,
      formData,
    );
    return response.data;

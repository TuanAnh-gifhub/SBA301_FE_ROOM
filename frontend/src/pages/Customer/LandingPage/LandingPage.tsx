...
const transformListingToRoom = (listing: Listing): Room | null => {
  ...
  return {
    ...
    logo:
      item.imageUrls && item.imageUrls.length > 0 ? item.imageUrls[0] : null,
    banner:
      item.imageUrls && item.imageUrls.length > 1 ? item.imageUrls[1] : null,
  };
};
    ...
    const fetchItems = async () => {
      try {
        ...
        if (
          ...
          console.log(
            "ℹ️ No listings data - showing template with empty state",
          );
          setRooms([]);
          setTotalItems(0);
          setStores([]);
      ...
      >
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            {stores
              .slice(0, showAllStores ? stores.length : 6)
              .map((store, idx) => (
                <motion.div
                  key={store.id}
                ...
                >
                  <div
                    className={`p-4 border-b ${isDarkMode ? "border-gray-700" : ""}`}
                  >
                  ...
                  <div className="relative bg-gray-200">
                      ...
                      <img
                        ...
                        onError={(e) => {
                          ...
                          const parent = (e.target as HTMLImageElement)
                            .parentElement;
                          const noBannerDiv = parent?.querySelector(
                    ...
                    <div
                      className={`w-full h-32 bg-linear-to-r from-gray-200 to-gray-300 flex items-center justify-center ${
                        store.banner ? "hidden" : "flex"
                      } no-banner-placeholder`}
                    >
        ...
        <div
          id="featured-stores"
          ref={featuredStoresRef}
          className="max-w-7xl mx-auto p-8"
        >
          <div className="text-center mb-12">
            <h1 className="text-xl md:text-2xl font-bold mb-4 text-[#4da6ff]">
              <ScrambleText
                text="Địa điểm cho thuê nổi bật"
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                  selectedCategory === String(category.id)
                    ? isDarkMode
                      ? "bg-[#4da6ff] text-white border border-[#4da6ff]"
                      : "bg-[#4da6ff] text-white border border-[#4da6ff]"
                    : isDarkMode
                      ? "bg-gray-700 text-gray-300 border border-gray-600 hover:border-[#4da6ff] hover:text-[#4da6ff]"
                      : "bg-white text-gray-700 border border-gray-300 hover:border-[#4da6ff] hover:text-[#4da6ff]"
                }`}
              >
                {category.name}
              </button>
          ...
          <div className="mb-8 w-full relative flex items-center">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6 w-full px-4 lg:px-12">
              {filteredStores
                .slice(featuredStoresStart, featuredStoresStart + 5)
                .map((store, idx) => (
                  <motion.div
                    key={store.id}
                    initial={{ opacity: 0, y: 20 }}
                  ...
                  >
                    <div className="flex flex-col items-center text-center h-full">
                      ...
                      <h3
                        className={`text-lg font-bold mb-2 transition-colors duration-300 ${isDarkMode ? "text-white group-hover:text-[#4da6ff]" : "text-gray-900 group-hover:text-[#4da6ff]"}`}
                      >
                        {store.name}
                      </h3>
                      ...
                      <div className="flex items-center mb-2">
                        ...
                        <span
                          className={`text-sm transition-colors duration-300 ${isDarkMode ? "text-gray-300 group-hover:text-gray-200" : "text-gray-600 group-hover:text-gray-700"}`}
                        >
                          {store.rating || 0} ({store.reviewCount || 0} đánh
                          giá)
                        </span>
                      </div>

                      <div
                        className={`text-xs mb-2 transition-colors duration-300 ${isDarkMode ? "text-gray-400 group-hover:text-[#4da6ff]" : "text-gray-500 group-hover:text-[#4da6ff]"}`}
                      >
                        Đang cho thuê: {store.currentListings || 0} | Đã cho
                        thuê: {store.soldItems || 0}
                      </div>

                      <div
                        className={`text-xs mb-3 flex-1 transition-colors duration-300 ${isDarkMode ? "text-gray-400 group-hover:text-[#4da6ff]" : "text-gray-500 group-hover:text-[#4da6ff]"}`}
                      >
                        📍 {store.location}
                      </div>

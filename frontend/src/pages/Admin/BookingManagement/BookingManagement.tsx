...
import SlotEditorModal from "../../Owner/ManageBookingPage/SlotEditorModal";
import UpdateBookingModal from "../../Owner/ManageBookingPage/UpdateBookingModal";
...
const ManageBookingPage = () => {
  ...
  const fetchBookings = async () => {
    try {
      ...
      if (status && status !== "ALL") params.bookingStatus = status;

      if (dates && dates.length === 2) {
        params.from = dates[0].startOf("day").format("YYYY-MM-DDTHH:mm:ss");
        params.to = dates[1].endOf("day").format("YYYY-MM-DDTHH:mm:ss");
      }

      const res = await getAllBookings(params);

      setData(res.result.data || res.result.content || []);
  ...
  const columns = [
    ...
    },
    {
      ...
      width: 80,
      render: (_: any, record: any) => {
        const currentStatus = record.bookingStatus;
